...
def diff_time(t1,t2):
    ...
    return abs(((t1[0]-t2[0])*60+t1[1]-t2[1])+t1[2]-t2[2])    
...
while True:
    ...
    while not_read:
        try:
            car = input("Car number")
            assert isinstance(car,int) is True
            not_read = False
    ...
    if len(car_time[car_number]) and cars_stats[car_number]==1:
        if diff_time(car_time[car_number][-1],time_now)<5: #under 15 mins, no need to show in the site
    ...
    prnt()
    print(time_now,txt_,cars_stats[car_number])
    print(car_time)
